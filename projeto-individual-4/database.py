...
def deletar_documento(doc_id: int) -> None:
    """Deleta um documento e seus registros associados (ON DELETE CASCADE)."""
    with closing(get_connection()) as conn:
        with conn:
            conn.cursor().execute("DELETE FROM documentos WHERE id = ?", (doc_id,))
